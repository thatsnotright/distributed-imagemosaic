...
import jimagemosaic.imageutils.*;
...
public class radiusRenderRandom extends renderTaskPlugin {
    ...
    private IImageList tileLibrary;       // The library of tiles we can pull from.
    ...
    public radiusRenderRandom (Color[][] ma, IImageList tileLib, RenderedImage baseImg, int h, int v, int pw, int ph, int ccp, int trt, int md, int to) {
...
/***************************************************************************/
        ...
        for ( int x = 0; x < hTiles; x++) {
        	for ( int y=0; y < vTiles; y++) {
        		String nextFi = tileLibrary.get(tileArray[x][y]).getFileName();
        		futures.put(nextFi,imageLoader.preFetch(nextFi));
        	}
        }
        Iterator<String> keyit = futures.keySet().iterator();
        while(keyit.hasNext()){
        	String key = keyit.next();
        	Future<String> future = futures.get(keyit);
        	if ( future == null || key == null )
        		continue;
        	try {
				imageLoader.add(key, future.get());
			} catch (Exception e) {
...
/****************************************************************/
            ...
            for (int y = 0; y < vTiles; y++) {  // Cycle through the horizontal tiles.
                try {
                    ...
                    if ( null == smallImage )
                    	System.out.println("FILE IS NULL! ZOMG!  "+tileLibrary.get(tileArray[x][y]).getFileName());
