...
public class medianEvaluator extends evaluatorPlugin {
    ...
    // Class variables.
    private IImageList images;  // The list of images we are evaluating.
    ...
    public medianEvaluator(IImageList il) {
        this.images = il;
                    ...
                    if (img != null) {
                        ...
                        if (mean.length == 3) {  // If there were 3 bands of Data (RBG, not Monochrome..)
                            Color c = new Color((int)mean[0], (int)mean[1], (int)mean[2]);  // Make a color from it.
                                if (c != null) {
                                    IImageInfo newImg = new imageInfo();         // Set up an imageInfo record with the data.
